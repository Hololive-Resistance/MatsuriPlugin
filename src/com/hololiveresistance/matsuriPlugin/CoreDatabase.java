...
import com.hololiveresistance.matsuriPlugin.Error; // YOU MUST IMPORT THE CLASS ERROR, AND ERRORS!!!
import com.hololiveresistance.matsuriPlugin.Errors;
import com.hololiveresistance.matsuriPlugin.MatsuriPlugin; // Import main class!

public abstract class CoreDatabase {
	MatsuriPlugin plugin;
    Connection connection;
    // The name of the table we created back in SQLite class.
    public String table = "co_container";
    public int tokens = 0;
    public CoreDatabase(MatsuriPlugin instance){
        plugin = instance;
    }
    ...
    public Integer getTotal() {
        ...
        try {
            conn = getSQLConnection();
            ps = conn.prepareStatement("SELECT time, user, x, y, z, type FROM " + table +";");
   
            rs = ps.executeQuery();
            while(rs.next()){
            	plugin.getLogger().info("user: "+rs.getString("user")+" x: "+rs.getString("x")+" y: "+rs.getString("y")+" z: "+rs.getString("z"));
            }
        } catch (SQLException ex) {
            plugin.getLogger().log(Level.SEVERE, Errors.sqlConnectionExecute(), ex);
        } finally {
